...
const BentoEducation = () => {
  ...
  return (
    <>
      ...
      <div
        ref={eduRef}
        className={`bento-card col-span-full lg:col-span-7 scroll-reveal-left ${eduVis ? "visible" : ""}`}
      >
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-3 sm:text-right shrink-0">
                  ...
                  <span className="text-xs font-mono text-accent">{edu.score}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div
        ref={certRef}
        className={`bento-card col-span-full lg:col-span-5 scroll-reveal-right ${certVis ? "visible" : ""}`}
      >
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {certifications.map((cert) => (
            <span key={cert} className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-xs font-medium">
              {cert}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {badges.map((badge, i) => (
            <div key={badge} className="flex items-center gap-3 p-2.5 rounded-xl bg-secondary/50">
      ...
      <div
        ref={pubRef}
        className={`bento-card col-span-full scroll-reveal-scale ${pubVis ? "visible" : ""}`}
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {publications.map((pub) => (
            <div key={pub} className="p-4 rounded-xl bg-gradient-to-br from-primary/5 to-accent/5 border border-border">
